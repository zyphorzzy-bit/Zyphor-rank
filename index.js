...
// ID do seu usuário para o comando z!servidores
...
client.on('ready', async () => {
    // Status Roxo de Streaming
    client.user.setActivity('zyphor app', {
        type: ActivityType.Streaming,
        url: 'https://www.twitch.tv/discord'
    });

    // Registrar o Slash Command /config globalmente
    ...
    console.log(`<:zyphor:1540096483276095621> Bot Público Zyphor Apps Online!`);
});

// Interação /config
client.on('interactionCreate', async (interaction) => {
    if (interaction.isChatInputCommand() && interaction.commandName === 'config') {
        const diasInput = interaction.options.getInteger('dias_reset') || 7;
            ...
            new StringSelectMenuBuilder()
                ...
                .addOptions([
                    ...
                    { 
                        ...
                        value: 'set_reset', 
                        description: `Atual: ${diasInput} dias`, 
        ...
        await interaction.reply({
            content: `<:zyphor:1540096483276095621> **Painel Zyphor Apps**\n<:horrio:1534611997335883886> Reset programado a cada **${diasInput}** dias.`,
            components: [row],
            ephemeral: true
        });
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'menu_config') {
        ...
        if (opcao === 'help_cmds') {
            const embedHelp = new EmbedBuilder()
                ...
                .addFields(
                    { name: '<:horrio:1534611997335883886> `z.rank call`', value: 'Exibe o ranking de tempo em voz + tempo de status roxo.' },
...
client.on('messageCreate', async (message) => {
    ...
    // Comando do Dono: z!servidores
    if (comando === 'z!servidores') {
        if (message.author.id !== DONO_ID) return;

        const totalServidores = client.guilds.cache.size;
        const totalMembros = client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0);

        const embedLogs = new EmbedBuilder()
            .setTitle('<:zyphor:1540096483276095621> Zyphor Apps - Logs de Servidores')
            .setColor('#2b2d31')
            .addFields(
                { name: '<:fixo:1541318082574684240> Servidores Conectados:', value: `\`${totalServidores}\` servidores`, inline: true },
                { name: '<:ID:1534611999085039786> Alcance de Membros:', value: `\`${totalMembros}\` usuários`, inline: true }
            )
            .setFooter({ text: 'Status de expansão do bot público' });

        return message.channel.send({ embeds: [embedLogs] });
    }
